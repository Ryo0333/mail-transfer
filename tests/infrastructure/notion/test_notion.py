...
class TestTitleExists:
    def test_returns_true_when_results_present(self, notion_client: NotionClient) -> None:
        ...
        with patch("src.infrastructure.notion.notion.httpx.Client", return_value=mock_client):
            result = notion_client._title_exists("Existing Title")
    ...
    def test_returns_false_when_results_empty(self, notion_client: NotionClient) -> None:
        ...
        # Act
        with patch("src.infrastructure.notion.notion.httpx.Client", return_value=mock_client):
            result = notion_client._title_exists("Nonexistent Title")
    ...
    def test_raises_on_api_error(self, notion_client: NotionClient) -> None:
        ...
        with patch("src.infrastructure.notion.notion.httpx.Client", return_value=mock_client):
            with pytest.raises(Exception, match="API Error"):
                notion_client._title_exists("Some Title")


class TestExport:
    def test_skips_export_when_title_exists(self, notion_client: NotionClient) -> None:
        # Arrange
        mail = Mail(subject="Existing Subject", sender="sender@example.com", body="Body")
        mock_client = _make_mock_http_client()

        # Act
        with patch.object(notion_client, "_title_exists", return_value=True) as mock_exists:
            with patch("src.infrastructure.notion.notion.httpx.Client", return_value=mock_client):
                notion_client.export(mail)

        # Assert
        mock_exists.assert_called_once_with(mail.subject)
        mock_client.post.assert_not_called()

    def test_posts_to_notion_when_title_not_exists(self, notion_client: NotionClient) -> None:
        # Arrange
        mail = Mail(subject="New Subject", sender="sender@example.com", body="Body")
        mock_client = _make_mock_http_client()

        # Act
        with patch.object(notion_client, "_title_exists", return_value=False):
            with patch("src.infrastructure.notion.notion.httpx.Client", return_value=mock_client):
                notion_client.export(mail)
    ...
    def test_export_includes_subject_as_title(self, notion_client: NotionClient) -> None:
        # Arrange
        mail = Mail(subject="My Title", sender="sender@example.com", body="Body content")
        mock_client = _make_mock_http_client()
